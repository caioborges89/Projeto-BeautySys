...
namespace BeautySys.Forms
{
    public partial class FrmCaixa_Abertura : Form
    {
        ...
        private void FrmCaixa_Abertura_Load(object sender, EventArgs e)
        {
            ...
            if (_caixaBO.verificarCaixaAbertoFechadoHoje())
            {
                DialogResult dialogResult = MessageBox.Show("Já existe um caixa aberto e fechado com a data de hoje, deseja reabri-lo?", "Caixa", MessageBoxButtons.YesNo);
                if (dialogResult == DialogResult.Yes)
                {
                    //reabrir caixa
                    if (_caixaBO.reabrirCaixa())
                    {
                        MessageBox.Show("Caixa reaberto com sucesso!", "Caixa", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
